...
use crate::commands::types::CallbackFunction;
...
fn create_golfer(args: &[&str]) {
    if args.len() < 2 {
        ...
        let password = sensitive_info_input("password");
    ...
    if args.is_empty() {
        ...
        for (_key, golfer_command) in get_golfer_commands() {
            println!(
                ...
                command_name = golfer_command.name,
                command_description = golfer_command.description
            );
        }
    } else {
        ...
        if golfer_cmds.contains_key(golfer_command_name) {
            ...
            println!(
                "golfer {golfer_command_name} is not a valid command - run 'golfer help' to see a list of valid commands."
            );
        }
    }
}
